...
public class AssistTree {
  ...
  protected static boolean addPlayerHelper(Player p, Node<Player> cur) {
    ...
    if (p.compareAssists(cur.getCont()) <= 0){
    ...
    else if (p.compareAssists(cur.getCont()) > 0) {
  ...
  public ArrayList<Player> lookupAllHelper(double minP, double maxP, Node<Player> cur){
    ...
    if (cur != null) {
      ...
      if (cur.getCont().getAssists() <= maxP && cur.getCont().getAssists() >= minP) {
      ...
      else if (cur.getCont().getAssists() > maxP) {
        if (cur.getLeft() != null) {
          return lookupAllHelper(minP, maxP, cur.getLeft());
        }
      }
      
      else if (cur.getCont().getAssists() < minP) {
        if (cur.getRight() != null) {
