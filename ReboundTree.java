...
public class ReboundTree {
  ...
  protected static boolean addPlayerHelper(Player p, Node<Player> cur) {
    ...
    if (p.compareRebounds(cur.getCont()) <= 0){
    ...
    else if (p.compareRebounds(cur.getCont()) > 0) {
  ...
  public ArrayList<Player> lookupAllHelper(double minP, double maxP, Node<Player> cur){
    ...
    if (cur != null) {
      ...
      if (cur.getCont().getRebounds() <= maxP && cur.getCont().getRebounds() >= minP) {
      ...
      else if (cur.getCont().getRebounds() > maxP) {
        if (cur.getLeft() != null) {
          return lookupAllHelper(minP, maxP, cur.getLeft());
        }
      }
      
      else if (cur.getCont().getRebounds() < minP) {
        if (cur.getRight() != null) {
