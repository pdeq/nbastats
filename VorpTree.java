...
public class VorpTree {
  ...
  protected static boolean addPlayerHelper(Player p, Node<Player> cur) {
    ...
    if (p.compareVorp(cur.getCont()) <= 0){
    ...
    else if (p.compareVorp(cur.getCont()) > 0) {
  ...
  public ArrayList<Player> lookupAllHelper(double minP, double maxP, Node<Player> cur){
    ...
    if (cur != null) {
      ...
      if (cur.getCont().getVorp() <= maxP && cur.getCont().getVorp() >= minP) {
      ...
      else if (cur.getCont().getVorp() > maxP) {
        if (cur.getLeft() != null) {
          return lookupAllHelper(minP, maxP, cur.getLeft());
        }
      }
      
      else if (cur.getCont().getVorp() < minP) {
        if (cur.getRight() != null) {
